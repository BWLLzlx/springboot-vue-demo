...
export default {
  ...
  methods:{
    ...
    save() {
      if(this.form.id) {//更新
        request.put("/api/man/manBook",this.form).then(res => {
          console.log(res)
          if(res.code === '0'){
            ElMessage('更新成功')
            sessionStorage.setItem("book",JSON.stringify(this.form))
            this.load();
          } else {
            ElMessage(res.msg)
          }
        ...
        request.post("/api/man/manBook",this.form).then(res => {
          console.log(res)
          if(res.code === '0'){
            ElMessage('新增成功')
            sessionStorage.setItem("book",JSON.stringify(this.form))
    ...
    handleDelete(id) {
      console.log(id)
      request.delete("/api/man/manBook/" + id).then(res => {
        if(res.code === '0'){
          ElMessage('删除成功')
          this.load();
        } else {
